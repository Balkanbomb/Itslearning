...
public class ExpandableListAdapter extends BaseExpandableListAdapter {
	...
	public ExpandableListAdapter(Context context, List<Article> listDataHeader) {
		...
		theCourses.add(new Course("1", context.getResources().getColor(
				R.color.blue)));
		theCourses.add(new Course("2", context.getResources().getColor(
				R.color.yellow)));
		theCourses.add(new Course("3", context.getResources().getColor(
				R.color.red)));
		theCourses.add(new Course("4", context.getResources().getColor(
				R.color.green)));
		theCourses.add(new Course("5", context.getResources().getColor(
				R.color.orange)));
	}
	...
	public View getChildView(int groupPosition, final int childPosition,
		...
		if (!_listDataHeader.isEmpty()) {
			if (convertView == null) {
				...
				convertView = infalInflater.inflate(R.layout.itsl_list_item,
						null);
			}

			TextView txtListChild = (TextView) convertView
					.findViewById(R.id.lblListItem);
			txtListChild.setText(this._listDataHeader.get(groupPosition)
					.getArticleText());
			ImageView imgClrCode = (ImageView) convertView
					.findViewById(R.id.clrCode);
			...
			for (Course c : theCourses) {
				if (this._listDataHeader.get(groupPosition)
						.getArticleCourseCode().equals(c.getCourseCode())) {
...

	{
		if (!_listDataHeader.isEmpty()) {
			if (convertView == null) {
				LayoutInflater infalInflater = (LayoutInflater) this._context
						.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
				convertView = infalInflater.inflate(R.layout.itsl_list_group,
						null);
			}
			...
			TextView lblListHeaderDate = (TextView) convertView
					.findViewById(R.id.lblListHeaderDate);
			TextView lblListHeaderText = (TextView) convertView
					.findViewById(R.id.lblListHeaderText);
			TextView lblListCode = (TextView) convertView
					.findViewById(R.id.lblListCode);
			ImageView imgClrCode = (ImageView) convertView
					.findViewById(R.id.clrCode);
			ImageView imgPointer = (ImageView) convertView
					.findViewById(R.id.icPointer);
			TextView txtClrLine = (TextView) convertView
					.findViewById(R.id.clrLine);
			...
			if (headerTitle.getArticleSummary().equals(
					headerTitle.getArticleText())) {
			...
			for (Course c : theCourses) {
				if (headerTitle.getArticleCourseCode()
